...
class DucumentUploadController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('Create Media')) {
            ...
            return redirect()->route('media-upload.index')->with('success', __('Document successfully uploaded.') . ((isset($result) && $result != 1) ? '<br> <span class="text-danger">' . $result . '</span>' : ''));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function destroy($id)
    {
        if (\Auth::user()->can('Delete Media')) {
            ...
            if ($document->created_by == \Auth::user()->creatorId()) {
                ...
                return redirect()->route('media-upload.index')->with('success', __('Document successfully deleted.'));
            } else {
                return redirect()->back()->with('error', __('Permission denied.'));
            }
        } else {
